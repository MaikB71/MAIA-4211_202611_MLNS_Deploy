...
- **Carga de archivos:** Permite subir archivos CSV con formato específico (separador: punto y coma [;]). 
    Estructura requerida: columna "ODS" (valor real) y columna "textos" (contenido a clasificar). La columna ODS es necesaria para el correcto funcionamiento, pero su contenido es opcional (Puede estar vacia).
...
if input_df is not None:
    ...
    if event is not None and event.selection.rows:        
        ...
        ODS_real = Y[current_row_index] if not pd.isna(Y[current_row_index]) else "No aplica"
